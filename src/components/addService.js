...
export default function BottomSheet (props){
    ...
    const [amount, Amount] = useState(null);
    const [imageuri, ImageUri] = useState(null);
    ...
    const Add =()=> {
        if ((name!=null)&&(amount!=null)){
            Date(new Date())
            DateText(new Date().toString().substring(0, 15))
            props.onDismiss({status:true,date:dateText.toString().slice(4,15),name:name,rate:amount,img:imageuri})
            Name(null)
            Amount(null)
            ImageUri(null)
        }
                ...
                <View>
                    <PanGestureHandler onGestureEvent={onGesture} onEnded={onGestureEnd}>
                        <View style={styles.header}>
                            <Text style={styles.heading}>New Service</Text>
                        </View>
                    </PanGestureHandler>
